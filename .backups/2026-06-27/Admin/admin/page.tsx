          ...
          <div className="admin-header-content-right">
            {activeTab !== 'dashboard' && (
              <button 
                ...
                style={{ marginRight: '16px', background: 'transparent', color: 'var(--text-color)', border: '1px solid var(--border-color)', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer' }}
              >
                ← Volver al Dashboard
              </button>
            )}
        ...
        {activeTab === 'dashboard' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px', marginTop: '20px' }}>
            <div className="dashboard-card" onClick={() => setActiveTab('users')}>
              ...
              <h3>Lista Blanca de Usuarios</h3>
              <p>Gestionar usuarios autorizados</p>
            </div>
            <div className="dashboard-card" onClick={() => setActiveTab('roles')}>
              <div className="card-icon">📊</div>
              <h3>Gestión de Roles</h3>
              <p>Permisos y niveles de acceso</p>
            </div>
            <div className="dashboard-card" onClick={() => setActiveTab('pending')}>
              <div className="card-icon">⏳</div>
              <h3>Usuarios en Suspenso {pendingUsers.length > 0 && <span style={{backgroundColor: '#C80036', color: 'white', padding: '2px 6px', borderRadius: '12px', fontSize: '11px', marginLeft: '8px'}}>{pendingUsers.length}</span>}</h3>
              <p>Aprobar nuevos registros</p>
            </div>
            <div className="dashboard-card" onClick={() => setActiveTab('ips')}>
              <div className="card-icon">🛡️</div>
              <h3>Firewall de IPs</h3>
              <p>Lista blanca de IPs permitidas</p>
            </div>
            <div className="dashboard-card" onClick={() => setActiveTab('templates')}>
              <div className="card-icon">✉️</div>
              <h3>Plantillas de Correo</h3>
              <p>Personalizar emails automáticos</p>
            </div>
            <Link href="/admin/auditoria" style={{ textDecoration: 'none' }}>
              <div className="dashboard-card">
                <div className="card-icon">⭐</div>
                <h3>Auditoría de Valoraciones</h3>
                <p>Monitoreo de feedback</p>
              </div>
            </Link>
