...
-- 2. Rename functions
ALTER FUNCTION update_chatbot_updated_at_column() RENAME TO update_paulita_updated_at_column;

-- 3. Rename triggers
ALTER TRIGGER trg_chatbot_conceptos_updated_at ON paulita_conceptos RENAME TO trg_paulita_conceptos_updated_at;

-- 4. Rename indexes
ALTER INDEX chatbot_embeddings_cosine_idx RENAME TO paulita_embeddings_cosine_idx;
-- If any other indexes were implicitly created for constraints, rename them too
ALTER INDEX chatbot_conceptos_pkey RENAME TO paulita_conceptos_pkey;
ALTER INDEX chatbot_concepto_simbolos_pkey RENAME TO paulita_concepto_simbolos_pkey;
