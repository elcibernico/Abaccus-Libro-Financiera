...
Partimos de la definición del valor actual () como la suma de las cuotas variables actualizadas, donde la época de valuación está ubicada en el momento inicial:
 $$V=\\frac{C}{(1+i)^{1}}+\\frac{C+r}{(1+i)^{2}}+\\frac{C+2r}{(1+i)^{3}}+…+\\frac{C+(n-2)r}{(1+i)^{n-1}}+\\frac{C+(n-1)r}{(1+i)^{n}}\\ => (1)$$`
    },
    {
      "type": "text",
...
$$$$
$$V\\cdot(1+i)=C+\\frac{C+r}{(1+i)^{1}}+\\frac{C+2\\cdot r}{(1+i)^{2}}+…+\\frac{C+(n-2)\\cdot r}{(1+i)^{n-2}}+\\frac{C+(n-1)\\cdot r}{(1+i)^{n-1}}\\ => (2)$$`
    },
    {
      "type": "text",
...
Procedemos a restar miembro a miembro la Ecuación (1) de la Ecuación (2). En el segundo miembro, restamos entre sí las fracciones que poseen exactamente el mismo denominador. Observa cómo la cuota base $C$ se anula en todos los términos centrales, sobreviviendo únicamente la razón $r$:
 $$V\\cdot(1+i)-V=\\left [C-\\frac{C}{(1+i)^{1}}\\right ]+\\left [\\frac{C+r}{(1+i)^{1}}-\\frac{C+r}{(1+i)^{2}}\\right ]+\\left [\\frac{C+2\\cdot r}{(1+i)^{2}}-\\frac{C+2\\cdot r}{(1+i)^{3}}\\right ]+…+$$
 $$\\left [\\frac{C+(n-2)\\cdot r}{(1+i)^{n-2}}-\\frac{C+(n-2)\\cdot r}{(1+i)^{n-1}}\\right ]+\\left [\\frac{C+(n-1)\\cdot r}{(1+i)^{n-1}}-\\frac{C+(n-1)\\cdot r}{(1+i)^{n}}\\right ]$$`
    },
    {
      "type": "text",
...
Sacando factor común $C$ en los primeros dos términos y factor común $r$ en el resto de los términos, a excepción del último:
 $$V\\cdot i=C\\cdot \\left [1-\\frac{1}{(1+i)^{n}}\\right ]+r\\cdot \\left [\\frac{1}{(1+i)^{1}}+\\frac{1}{(1+i)^{2}}+…+\\frac{1}{(1+i)^{n-1}}\\right ]-\\frac{(n-1)r}{(1+i)^{n}}$$`
    },
    {
      "type": "text",
...
Dentro del corchete que multiplica a $r$ , tenemos una serie geométrica que llega solamente hasta el exponente $(n-1)$ . Para que represente una renta completa de $n$ períodos, debemos sumarle el término faltante $\\frac{1}{(1+i)^{n}}$ y, para no alterar la ecuación, restarlo inmediatamente fuera de la sumatoria:
 $$V\\cdot i=C\\cdot \\left [1-\\frac{1}{(1+i)^{n}}\\right ]+r\\cdot \\left [\\frac{1}{(1+i)^{1}}+\\frac{1}{(1+i)^{2}}+…+\\frac{1}{(1+i)^{n-1}}+\\frac{1}{(1+i)^{n}}-\\frac{1}{(1+i)^{n}}\\right ]-\\frac{(n-1)\\cdot r}{(1+i)^{n}}$$
$$V\\cdot i=C\\cdot \\left [1-\\frac{1}{(1+i)^{n}}\\right ]+r\\cdot \\left [\\frac{1}{(1+i)^{1}}+\\frac{1}{(1+i)^{2}}+…+\\frac{1}{(1+i)^{n-1}}+\\frac{1}{(1+i)^{n}}\\right ]-r\\cdot \\frac{1}{(1+i)^{n}}-\\frac{(n-1)\\cdot r}{(1+i)^{n}}$$
$$V\\cdot i=C\\cdot \\left [1-\\frac{1}{(1+i)^{n}}\\right ]+r\\cdot \\sum_{j=1}^{n}\\frac{1}{(1+i)^{j}}-\\frac{r}{(1+i)^{n}}-\\frac{(n-1)\\cdot r}{(1+i)^{n}}$$ 
...
La ecuación queda sintetizada en:
 $$V\\cdot i=C\\cdot (1-\\frac{1}{(1+i)^{n}})+r\\cdot \\sum_{j=1}^{n}\\frac{1}{(1+i)^{j}}-\\frac{n\\cdot r}{(1+i)^{n}}$$`
    },
    {
      "type": "text",
...
$$V_{n_{a}}=\\left [C+\\frac{r}{i}+n\\cdot r\\right ]\\cdot a_{n:i}-\\frac{n\\cdot r}{i}$$`
    },
    {
      "type": "text",
...
Vamos a separar el término $n\\cdot r$ del resto dentro del paréntesis y distribuimos $a_{n;i}$ :
 $$V_{n_{a}}=\\left (C+\\frac{r}{i})\\cdot a_{n:i}+(n\\cdot r\\right )\\cdot a_{n:i}-\\frac{n\\cdot r}{i}$$ 
...
Tomamos los dos últimos términos y sacamos factor común $n\\cdot r$ :
 $$n\\cdot r\\cdot \\left (a_{n:i}-\\frac{1}{i}\\right )$$ 
...
Ahora multiplicamos este resultado por el factor común $n\\cdot r$ que habíamos dejado afuera en el Paso 2:
 $$n\\cdot r\\cdot \\left [-\\frac{1}{i\\cdot (1+i)^{n}}\\right ]=-\\frac{r}{i}\\cdot \\frac{1}{(1+i)^{n}}\\cdot n$$ 
...
Nuevamente, si se tratase de una renta de cuotas adelantadas, sencillamente le entregamos a la ecuación (fórmula) el factor de cuota adelantada $(1+i)$ y en la simbología del valor financiero de la renta, agregamos el apóstrofo $V'_{n_{a}}$ . Así:
 $$V'_{n_{a}}=\\left[\\left (C+\\frac{r}{i}\\right )\\cdot a_{n:i}-\\frac{r}{i}\\cdot \\frac{1}{(1+i)^{n}}\\cdot n\\right]\\cdot (1+i)$$`
    }
  ],
  "Glosario": [],
  "Casos Prácticos": [],
